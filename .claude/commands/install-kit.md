...
- To refresh `~/.claude/dev-kit-config.json` with the current kit path (e.g., after moving the kit repo)
...
4. Writes `~/.claude/dev-kit-config.json` with current kit path and timestamp
...
- Does not sync project-level config (`_claude-project/` is for consumer projects, consumed via `/sync-dev-kit`)
