...
- **✨ `claude-design` skill and `/ui-design`** - A design's life in Claude Design: `start` talks a screen through in its own folder, `prototype` builds it, `feedback` works reviewers' comments one at a time, `implement` builds the real screens and removes the folder. `publish-system` builds the project's design system from code — tokens, type roles, spacing, radii, fonts and the real components (bundle, previews, guidelines), resolving `color-mix()` and theme blocks exactly and failing on anything it cannot translate — and `refresh-design` brings designs onto the current system. The bundle runs on the React 18 that design pages use, and a render check mounts every component in light, dark and canvas mode. `project-documentation/ui-design-cheatsheet.md` is the one-page reference.
- **📝 Token and React rules for Claude Design** - The `design-system` skill requires every token to be resolvable and commented, and design-system components to stay inside what React 18 and 19 share.
...
- **🐛 Dev-kit sync auto-merge race** - `/sync-dev-kit` now waits for required GitHub status checks to register before admin-merging the sync PR, avoiding the "N of N expected" failure.
...
- **🐛 Sync auto-merge bypass** - `/sync-dev-kit` now merges its own PR with admin bypass and fails loud if the merge is blocked, preventing a stale-main trap where primary returned to main while the sync PR sat unmerged behind branch-protection checks.
...
- **✨ Biome Template for AI-Authored TS/JS Projects** - New `biome.json` ships at the consumer repo root via `/sync-dev-kit`. Formatter disabled (AI-authored code doesn't need formatting consistency churn), linter on `"recommended": true`, scoped with `vcs.useIgnoreFile` so build outputs are excluded via the project's `.gitignore`. Paired with a new commit-time Biome lint step in gitflow's `/commit` so failures fire in <1 second locally instead of 30 seconds on the PR.
...
- **✨ Kit-Shipped GitHub Actions Templates** - New `_github-project/workflows/` directory ships `version-bump.yml` (TypeScript + Python support) and `commitlint.yml` to consumer projects via `/sync-dev-kit`. Convention: `_<name>/` maps 1:1 to `.<name>/` in the consumer.
