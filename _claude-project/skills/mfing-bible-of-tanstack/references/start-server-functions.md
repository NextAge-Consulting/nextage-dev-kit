<!-- VENDORED — do not hand-edit.
     source:   @tanstack/start-client-core@1.170.17 :: skills/start-core/server-functions/SKILL.md
     vendored: 2026-08-06 by the kit (/review-tanstack refreshes it)
     Upstream relative links were rewritten to this folder's flat names.
     Local edits are overwritten on refresh — put house guidance in SKILL.md. -->
...
Apply `authMiddleware` (or an equivalent in-handler check) to **every** `createServerFn` that needs auth. See [start-core/auth-server-primitives](./start-auth-server-primitives.md) for the full session/middleware pattern and [start-core/middleware](./start-middleware.md) for composing the factory.
...
- [start-core/execution-model](./start-execution-model.md) — understanding where code runs
- [start-core/middleware](./start-middleware.md) — composing server functions with middleware
- [start-core/auth-server-primitives](./start-auth-server-primitives.md) — sessions, cookies, OAuth, CSRF, rate limiting (the server-side half of auth; `getCurrentUser`/`useSession`-style helpers belong here, not at module scope)
- [router-core/auth-and-guards](./router-auth-and-guards.md) — routing-side UX guards; data auth belongs in the server function, server route, or API endpoint handler/middleware
