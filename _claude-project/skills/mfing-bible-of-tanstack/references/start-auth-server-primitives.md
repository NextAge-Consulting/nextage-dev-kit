<!-- VENDORED — do not hand-edit.
     source:   @tanstack/start-client-core@1.170.17 :: skills/start-core/auth-server-primitives/SKILL.md
...
This skill covers the **server half** of authentication: session storage, cookie issuance, OAuth flow, password-reset hardening, CSRF, rate limiting. For the **routing half** (`_authenticated` layout, `beforeLoad` redirects, RBAC checks), see [router-core/auth-and-guards](./router-auth-and-guards.md).
...
On Cloudflare Workers and other edge runtimes, the module-level read evaluates to `undefined` even on the server because env is injected per-request. See [start-core/execution-model](./start-execution-model.md).
...
- [router-core/auth-and-guards](./router-auth-and-guards.md) — the routing side: `_authenticated` layout, `beforeLoad`, `redirect`, RBAC checks.
- [start-core/server-functions](./start-server-functions.md) — how to expose RPCs (and how the route guard does NOT cover them).
- [start-core/middleware](./start-middleware.md) — composing `authMiddleware` and others.
- [start-core/execution-model](./start-execution-model.md) — why module-level env/secret reads are wrong.
