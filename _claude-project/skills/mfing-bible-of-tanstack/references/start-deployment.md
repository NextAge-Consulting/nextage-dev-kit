<!-- VENDORED — do not hand-edit.
     source:   @tanstack/start-client-core@1.170.17 :: skills/start-core/deployment/SKILL.md
...
> ```
>
> Or read `process.env.X` per-request inside `.handler()` / middleware `.server()`. See [Cloudflare's environment-variables docs](https://developers.cloudflare.com/workers/configuration/environment-variables/) and [start-core/execution-model](./start-execution-model.md).
...
- [start-core/server-routes](./start-server-routes.md) — API endpoints for sitemaps, robots.txt
- [start-core/execution-model](./start-execution-model.md) — SSR affects where code runs
